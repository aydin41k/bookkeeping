...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">Payment Balances</div>
                <div class="card-body">
                    @empty( $dr )
                    ...
                    @else
                        <div class="row border border-top-0 border-right-0 border-left-0">
                            ...
                            <div class="col-md-2"><strong>Total purchases</strong></div>
                            <div class="col-md-2"><strong>Total payments</strong></div>
                            <div class="col-md-2"><strong>Balance</strong></div>
                        </div>
                        @foreach( $dr as $d )
                        @if( $d->purchaseinvoice->sum('amount') )
                        <div class="row lineitem">
                            <div class="col-md-6"><a href="{{ action('CompanyController@balance',['id'=>$d->id,'account'=>'cr']) }}">{{ $d->name }}</a></div>
                            <div class="col-md-2 sale text-right">{{ number_format($d->purchaseinvoice->sum('amount'),2,'.','') }}</div>
                            <div class="col-md-2 payment text-right">{{ number_format($d->payments->sum('amount'),2,'.','') }}</div>
